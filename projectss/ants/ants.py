...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        location = self.place
        curr_distance = 0
        while location.entrance is not None:
            if curr_distance > self.upper_bound:
                return None
            if not location.is_hive:
                if len(location.bees) > 0 and curr_distance >= self.lower_bound:
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        exiter = self.place.exit
        while exiter:
            if exiter.ant is not None:
                if not exiter.ant.doubled:
                    exiter.ant.doubled = True
                    exiter.ant.double()
                if exiter.ant.is_container == True and exiter.ant.ant_contained is not None:

                    if not exiter.ant.ant_contained.doubled:
                        exiter.ant.ant_contained.double()
                        exiter.ant.ant_contained.doubled = True
            exiter = exiter.exit
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        # BEGIN Problem EC
        default = 5
        target.turns_left = default

        def action(gamestate):
            if target.turns_left:
                if gamestate.time % 2 == 0:
                    Bee.action(target,gamestate)
                target.turns_left = target.turns_left - 1
            else:
                Bee.action(target,gamestate)
