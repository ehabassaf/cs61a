...
def about(topic):
    ...
    def select(paragraph):
        newlist = remove_punctuation(paragraph)
        newlist = lower(newlist)
        newlist = split(newlist)
        if list(set(topic) & set(newlist)):
            return True
        else:
            return False
    return select
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for i in range(len(word_list)):
        val = diff_function(typed_word, word_list[i], limit)
        storage = storage + [val] 
    if len(storage) == 0:
        return word_list[0]
    if min(storage) <= limit:
        return word_list[storage.index(min(storage))]
    if min(storage) > limit:
        return typed_word 
    # END PROBLEM 5
...
def time_per_word(words, times_per_player):
    ...
    for i in range(len(times_per_player)):
        templist = []
        for n in range(len(times_per_player[i])-1):
            value = times_per_player[i][n+1] - times_per_player[i][n]
            templist.append(value)
        updated_times.append(templist)
    return match(words,updated_times)
    # END PROBLEM 9
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            if time(match, j, i) < min_time:
                ...
                player_num = j
        fastest[player_num].append(get_word(match, i))
    return fastest
