...
def is_always_roll(strategy, goal=GOAL):
    ...
    """
    # BEGIN PROBLEM
    if 'always_roll' in str(strategy):
        return True
    else: 
        return False
...
def make_averaged(original_function, total_samples=1000):
    ...
    def average(*args):  
        ...
        while n < total_samples:
            count = count + original_function(*args)
            n = n + 1
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    maxrolls = 0  
    while n > 0:
        averagescore = make_averaged(roll_dice,1000)(n,dice)
        maxscore = max(maxscore, averagescore)
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    if perfect_square(score + x):
        z = square_update(num_rolls, score + x, opponent_score, dice=six_sided)
