...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        shorthand = do_lambda_form(Pair(signature.rest, expressions.rest), env)
        env.define(signature.first, shorthand)
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        return True
    changed_value = scheme_eval(expressions.first, env)
    if is_scheme_true(changed_value):
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        binding_first = bindings.first
        validate_form(binding_first, 2, 2)
        names = Pair(binding_first.first, names) 
        vals = Pair(scheme_eval(binding_first.rest.first, env), vals)
